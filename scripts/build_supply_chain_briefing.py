...
DAILY_THEMES = [
    {
        "tech": ("AI 驱动的供应链例外管理", "优先梳理延误、缺料和需求异常的识别规则，并把人工审批嵌入执行链路。", "AI-enabled supply chain exception management", "Prioritize rules for delays, shortages, and demand anomalies, with human approval embedded in execution."),
        "industry": ("端到端交期风险监控", "将承运人 ETA、港口拥堵、清关状态和工厂产能汇入同一例外清单。", "End-to-end lead-time risk monitoring", "Bring carrier ETA, port congestion, customs status, and plant capacity into a single exception list."),
        "explainer": ("供应链控制塔", "控制塔汇集跨部门、跨企业数据，用于发现例外、评估影响并协调行动。", "Control Tower", "A control tower unifies cross-functional and partner data to detect exceptions, assess impact, and coordinate action."),
    },
    {
        "tech": ("需求感知（Demand Sensing）", "将销售、渠道、促销和外部信号纳入短期预测，重点监控需求突变而非只延长历史时间序列。", "Demand sensing", "Combine sales, channel, promotion, and external signals in short-term forecasts, with emphasis on detecting demand shifts."),
        "industry": ("多式联运的交付韧性", "为关键航线预先设计海运、铁路、公路和空运的切换条件，并把成本、时效和碳排放写入同一决策规则。", "Resilience through multimodal transport", "Predefine switching conditions across ocean, rail, road, and air, using one decision rule for cost, lead time, and emissions."),
        "explainer": ("安全库存", "安全库存用于吸收需求波动和补货提前期的不确定性，不是简单地把库存越做越高。", "Safety Stock", "Safety stock absorbs uncertainty in demand and replenishment lead time; it is not simply an instruction to hold more inventory."),
    },
    {
        "tech": ("仓储机器人编排", "将 AMR、拣选站和 WMS 任务波次协同调度，优先衡量订单周期和异常恢复时间，而非单台设备利用率。", "Warehouse robot orchestration", "Coordinate AMRs, picking stations, and WMS waves; prioritize order cycle time and recovery from exceptions over individual utilization."),
        "industry": ("供应商风险分级", "按单一来源程度、替代周期、财务健康度和地缘暴露度评估关键供应商，并绑定缓解动作和复核频率。", "Supplier risk tiering", "Assess critical suppliers by single-source exposure, substitution lead time, financial health, and geopolitical exposure, with assigned mitigations."),
        "explainer": ("总拥有成本（TCO）", "TCO 将采购价之外的运输、库存、质量、关税和风险成本纳入比较，是采购决策比单价更完整的视角。", "Total Cost of Ownership", "TCO compares options beyond purchase price by including transport, inventory, quality, duty, and risk costs."),
    },
]
...
def build_page(now, technology, industry):
    ...
    theme = DAILY_THEMES[now.toordinal() % len(DAILY_THEMES)]
    tech = news_cards(technology, theme["tech"][:2], theme["tech"][2:], "影响/启示：评估自动化建议对计划、仓储与运输约束的影响。", "Implication: Assess how automated recommendations affect planning, warehousing, and transport constraints.")
    industry_html = news_cards(industry, theme["industry"][:2], theme["industry"][2:], "影响/启示：用订单级影响而非新闻热度决定升级优先级。", "Implication: Prioritize escalation by order-level impact, not headline volume.")
